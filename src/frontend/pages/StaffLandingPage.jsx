...
import { ApplicationCard , FilterBar} from "@/components/ui";
...
const Index = () => {
    ...
    return mockApplicationsReview
      .filter((app) => {
        ...
        const matchesSearch =
          ...
          (app.businessName
            ? String(app.businessName).toLowerCase().includes(q)
            : false);
        ...
        const matchesProduct =
          productFilter === "all" || app.productType === productFilter;

        return matchesSearch && matchesRisk && matchesStatus && matchesProduct;
      })
      .sort(
        (a, b) =>
          (riskPriority[a.riskLevel] ?? 999) -
          (riskPriority[b.riskLevel] ?? 999),
      );
  ...
  const criticalCount = mockApplicationsReview.filter(
    (a) => a.riskLevel === "critical",
  ).length;
  const highCount = mockApplicationsReview.filter(
    (a) => a.riskLevel === "high",
  ).length;
      ...
      <main className="container mx-auto px-6 py-12">
        ...
        <div className="mb-8">
          ...
          <p className="text-muted-foreground">
            Review and process high-risk applications requiring manual
            verification
          </p>
        </div>
          ...
          <div className="mb-6 p-4 bg-risk-critical/10 border border-risk-critical/20 rounded-lg flex items-center gap-3">
            ...
            <div>
              <p className="font-medium text-foreground">
                {criticalCount} critical and {highCount} high-risk applications
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          ...
          <div className="p-4 bg-card border rounded-lg border-l-4 border-l-status-warning">
            ...
            <p className="text-2xl font-bold text-status-warning">
              {
                mockApplicationsReview.filter(
                  (a) => a.status === "awaiting_resubmission",
                ).length
              }
            </p>
          </div>
