...
def setup_db(job, app):
    ...
    # First db.query(...) call is for ReviewJobs
    # Second db.query(...) call is for ApplicationForm
...
@patch("backend.compliance_rules_engine.review_service.SessionLocal")
def test_sdd_approve(mock_session, mock_audit, mock_submit, mock_approve):
    job = make_job()
...
@patch("backend.compliance_rules_engine.review_service.create_audit_log")
@patch("backend.compliance_rules_engine.review_service.SessionLocal")
def test_sdd_kyc_declined(mock_session, mock_audit, mock_submit, mock_manual):
    job = make_job()
...
def test_cdd_edd(mock_session, mock_audit, mock_submit, mock_manual, decision):
    job = make_job()
    app = make_app()
...
@patch("backend.compliance_rules_engine.review_service.create_audit_log")
@patch("backend.compliance_rules_engine.review_service.SessionLocal")
def test_auto_reject(mock_session, mock_audit, mock_submit, mock_reject):
    job = make_job()
    app = make_app()
...
@patch("backend.compliance_rules_engine.review_service.submit_application")
@patch("backend.compliance_rules_engine.review_service.SessionLocal")
def test_exception_handling(mock_session, mock_submit):
    job = make_job()
    app = make_app()
    ...
    # 1st query: fetch ReviewJobs with for_update
    # 2nd query: fetch ApplicationForm
    # 3rd query: fetch failed_job inside except block
