...
from backend.services.gemini_extractor import (
    ...
    classify_document,
    parse_generic_additional_document,
...
def _normalize_requested_doc_match(match_result: Dict[str, Any], requested_document_name: str) -> bool:
    requested = (requested_document_name or "").strip().lower()
    guessed = (match_result.get("best_guess_document_type") or "").strip().lower()
    reason = (match_result.get("reason") or "").strip().lower()
    confidence = (match_result.get("confidence") or "").strip().upper()

    llm_flag = bool(match_result.get("matches_requested_document", False))

    semantic_text_signals_match = any(
        phrase in reason
        for phrase in [
            "directly matches",
            "reasonably matches",
            ...
            "appears to match",
            "is a match",
            "matches the requested document",
        ]
    )

    same_name = bool(requested and guessed and requested == guessed)

    if not llm_flag and confidence == "HIGH" and (same_name or semantic_text_signals_match):
        return True

    return llm_flag


@router.post("/extract-additional-document")
async def extract_additional_document(
    file: UploadFile = File(...),
    requested_document_name: str = Form(...),
):
    try:
        _, doc_ai_result, raw_text = await _run_initial_document_processing(file)
        ...
        if ocr_quality_assessment.get("quality_score", 100) < OCR_WARNING_THRESHOLD:
            ...
            summarized_upload_validation["reasons"] = [
                "Document quality is moderate. Please verify the extracted information carefully."
            ]

        match_result = assess_requested_document_match(
            raw_text=raw_text,
            requested_document_name=requested_document_name,
        )

        matches_requested_document = _normalize_requested_doc_match(
            match_result=match_result,
            requested_document_name=requested_document_name,
        )

        if not matches_requested_document:
            summarized_upload_validation["status"] = "FAIL"
            ...
            summarized_upload_validation["reasons"] = [
                f"Uploaded file does not match the requested document '{requested_document_name}'."
            ]
        ...
        final_data = parse_generic_additional_document(
            raw_text=raw_text,
            requested_document_name=requested_document_name,
        )
